...
import { BsInfoLg, BsSortDownAlt, BsSearch } from 'react-icons/bs';
...
import PokemonList from 'components/tool/PokemonList';
...
const ListWrapper = styled.div`
  width: 100%;
  max-width: 360px;
  margin-left: max(calc(50% - 180px), 0px);
  height: 100%;
`;
...
const SearchButtonWrapper = styled.div`
  ...
  z-index: 5;
`;
const SpecButtonWrapper = styled.div`
  position: absolute;
  top: -20px;
  left: calc(100% - 85px);
  width: 40px;
  ...
  border: 2px solid white;
  border-color: ${(props) => (props.popUp ? 'white' : '#ffffff99')};
  background-color: ${(props) => (props.popUp ? '#244291' : '#4252e7')};
  opacity: ${(props) => (props.popUp ? 1 : 0.75)};
  transition: all 0.2s linear;
  cursor: pointer;
  z-index: 5;
`;
const StatButtonWrapper = styled.div`
  position: absolute;
  top: -20px;
  left: calc(100% - 40px);
  width: 40px;
  ...
  border: 2px solid white;
  border-color: ${(props) => (props.popUp ? 'white' : '#ffffff99')};
  background-color: ${(props) => (props.popUp ? '#141a40' : '#6f7ccf')};
  opacity: ${(props) => (props.popUp ? 1 : 0.75)};
  transition: all 0.2s linear;
  cursor: pointer;
  z-index: 5;
...
const AppForSmallScreen = () => {
  ...
  const [bottomPopUp, setBottomPopUp] = useState(0);

  return (
    <Background>
      <Wrapper>
        <TopScreen>
          {topPopUp === 0 && <ScreenWrapper>{data ? <PokemonInfo /> : <Default2 />}</ScreenWrapper>}
          {topPopUp === 1 && <ScreenWrapper>{data ? <PokemonSpec /> : <Default2 />}</ScreenWrapper>}
          {topPopUp === 2 && <ScreenWrapper>{data ? <PokemonStat /> : <Default2 />}</ScreenWrapper>}
          <LanguageButton />
          <FirstSortButton />
          <SecondSortButton popUp={bottomPopUp} setPopUp={setBottomPopUp} />
          <SearchButton popUp={bottomPopUp} setPopUp={setBottomPopUp} />
          {data && <SpecButton popUp={topPopUp} setPopUp={setTopPopUp} />}
          {data && <StatButton popUp={topPopUp} setPopUp={setTopPopUp} />}
        </TopScreen>
        <BottomScreen>
          {bottomPopUp === 0 && <ListWrapper>{list ? <PokemonList /> : <Loading r="100px" />}</ListWrapper>}
          {bottomPopUp === 1 && (
            <ScreenWrapper>
              <SecondSort />
            </ScreenWrapper>
...
export default AppForSmallScreen;
...
const SecondSortButton = ({ popUp, setPopUp }) => {
  return (
    <SecondSortButtonWrapper popUp={popUp === 1} onClick={() => setPopUp(popUp === 1 ? 0 : 1)}>
      <BsSortDownAlt />
...
const SearchButton = ({ popUp, setPopUp }) => {
  return (
    <SearchButtonWrapper popUp={popUp === 2} onClick={() => setPopUp(popUp === 2 ? 0 : 2)}>
      <BsSearch />
    </SearchButtonWrapper>
  );
};
const SpecButton = ({ popUp, setPopUp }) => {
  return (
    <SpecButtonWrapper popUp={popUp === 1} onClick={() => setPopUp(popUp === 1 ? 0 : 1)}>
      <BsInfoLg />
    </SpecButtonWrapper>
  );
};
const StatButton = ({ popUp, setPopUp }) => {
  return (
    <StatButtonWrapper popUp={popUp === 2} onClick={() => setPopUp(popUp === 2 ? 0 : 2)}>
      <BiStats />
    </StatButtonWrapper>
  );
};
